...
{
    // recebe o parametro id vindo da navegacao quando o produto ja existe
    [QueryProperty(nameof(IdProduto), "id")]
    public partial class NovoProduto : ContentPage // tela de cadastro e edicao de produtos
    {
        ...
        private async Task PreencherCamposSeEdicao()
        {
            ...
            {
                // se der problema pra carregar os dados, avisa o usuario com uma mensagem simples
                await DisplayAlert("Ops", "Nao consegui carregar o produto. Tente de novo", "OK");
            }
        }
        ...
        private async void AoSalvarProduto(object sender, EventArgs e)
        {
            ...
            {
                // se der erro pra salvar, mostra uma mensagem simples e nao sai da tela
                // assim o usuario pode conferir os dados e tentar de novo
                await DisplayAlert("Ops", "Nao consegui salvar o produto. Confira os dados e tente de novo", "OK");

                return; // encerra o metodo sem voltar pra tela anterior
            }
